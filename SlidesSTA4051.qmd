...
`Answer` We have $n'=18$ pairs, each one for a worker:
$$ (RTBL,RTAL) = (X,Y)$$
- `"+"` if $RTBL < RTAL$ (12)
- `"-"` if $RTBL > RTAL$ (5)
...
## Contingency Tables - Chi-squared Test for differences in Probabilities
...
| **`Totals`**  | $c_1$ = 30 |   $c_2$= 130 | N = 160  |
 ...
 `Question`: Test whether there are difference in proportions of defective items between the two carloads.
...
## Chi-squared Test - Example with R
